#!/usr/bin/env python3
"""
Analyze Wikipedia pages using both KL divergence and DEFLATE compression
"""
...
class WikipediaContentAnalyzer:
    ...
    def load_wikipedia_pages(self):
        ...
        for file_path in self.wikipedia_dir.glob("*.txt"):
            ...
            try:
                ...
                if item_name.startswith('president_'):
                    category = 'president'
                    display_name = item_name.replace('president_', '').title()
                elif item_name.startswith('country_'):
                    category = 'country'
    ...
    def run_full_analysis(self):
        ...
        print("Starting Wikipedia content analysis...")
        
        # Load all content
        self.load_wikipedia_pages()
        ...
        president_count = sum(1 for cat in self.categories.values() if cat == "president")
        country_count = sum(1 for cat in self.categories.values() if cat == "country")
        print(f"  US Presidents: {president_count}")
        ...
        # Save results
        output_path = self.output_dir / "wikipedia_analysis.json"
...
def main():
    ...
    if results:
        ...
        print(f"US Presidents: {results['metadata']['presidents']}")

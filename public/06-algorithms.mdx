...
Our general investing strategy is this: We start with a uniform distribution $p_1^{(0)}, \dots, p_n^{(0)}, p_i^{(0)} = 1/n$ over the experts. At the beginning of each day, we sample an expert from this distribution and follow their advice. At the end of the day, we look at the losses $g_i^{(t)}$ and update $p^{(t)}$ to $p^{(t+1)}$.
...
Where $\eps$ is the [learning rate](https://en.wikipedia.org/wiki/Learning_rate) of the algorithm. After doing this update, we would of course normalize the probabilities so that they sum up to one. 
If we use $G_i^{(t)}$ for the total accumulated gain, i.e., $G_i^{(t)} = g_i^{(1)} + \dots + g_i^{(t)}$, then we can rewrite <EqRef id = "gradient"/> simply as

<Math displayMode={true} math = "p_i^{(t+1)} \propto \eps \cdot G_i^{(t)}" />
Notice that the learning rate does not play a role due to the normalization, so we end up with the proportional sampling rule from the riddle statement:

<Math displayMode={true} math = "p_i^{(t+1)} \propto G_i^{(t)}" />


This proportional rule has some problems - in the widget, you can notice that if the first expert has gained $100 and the other two experts gained $50, the proportional rule goes to the top expert only with 50% probability, although it should better go for her with close to 100% probability. 

### Follow the leader 
Maybe sampling the next expert is not the right approach and we should simply pick the best expert so far? This algorithm - called follow the leader - is very natural and works mostly very well. There are some weird scenarios though when it behaves poorly - see the widget. 


### Multiplicative weights update
Multiplicative weights update algorithm is like gradient descent rule <EqRef id = "gradient"/>, but the updates are multiplicative instead of additive:

<Math displayMode={true} math = "p_i^{(t+1)} \propto p_i^{(t)} \cdot e^{- \eps \cdot g_i^{(t)}}" />

This algorithm interpolates between gradient descent (the limit of this algorithm for $\eps \righarrow 0$) and follow the leader (the limit for $\eps \rightarrow \infty$). 

If we set up $\eps$ in the right way, this turns out to be an amazing algorithm combining the strengths of both approaches. 
The bonkers property is that if you run multiplicative weights algorithm for $t$ steps and set $\eps$ to be about $1/\sqrt{t}$, the algorithm is _always almost as good as the best expert_! In particular, it gains at most $O(\sqrt{t})$ dollars less than the best expert in expectation. <Footnote>Assuming all gains are between $0$ and $1$. If there are $n$ experts, the algorithm lack at most <Math math = "O(\sqrt{t\log n})" /> dollars in expectation. Check out [this excellent survey](https://www.cs.princeton.edu/~arora/pubs/MWsurvey.pdf) for more math behind the algorithm and its uses in computer science. </Footnote> In other words, whatever sequence of expert gains we could come up with in our widget, MWU would always end up on top. 
...
Let's think about all the information we learn in the first $t$ steps of our investing game. Each expert $i$ accumulated a total gain $G_i^{(t)} = g_i^{(1)} + \dots + g_i^{(t)}$. How should this total loss translate to the probability that we sample them in the $t+1$-th round?

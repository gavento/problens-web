...
<Math displayMode={true} math = "p_i^{(t+1)} = p_i^{(t)} - \varepsilon \cdot \ell_i^{(t)}"/>
...
<Math displayMode={true} math = "p_i^{(t+1)} \propto p_i^{(t)} \cdot e^{- \varepsilon \cdot \ell_i^{(t)}}" />

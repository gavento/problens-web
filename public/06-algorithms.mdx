...
In the [chapter about machine learning](05-machine_learning), we've seen how KL divergence guides us all the way from having a rough guess about our data to a well-defined optimization problem with a concrete loss function. This is where KL divergence really shines and shows us the way.
...
Say we want to get rich by investing in the stock market. Fortunately, there are $n$ investors willing to share their advice with us: Each day $t$, they give us some advice, and at the end of the day, we learn how good that advice was -- for the $i$-th expert, we'll have loss $\ell_i^{(t)}$ <Footnote>If this number is positive, the advice was bad; if it's negative, the advice was good.</Footnote>

Our general investing strategy is this: We start with a uniform distribution $p_1^{(0)}, \dots, p_n^{(0)}, p_i^{(0)} = 1/n$ over the experts. At the beginning of each day, we sample an expert from this distribution and follow their advice. At the end of the day, we look at the losses $\ell_i^{(t)}$ and update $p^{(t)}$ to $p^{(t+1)}$.

The question is: how should we update?

One way is using gradient descent:

<Math displayMode={true} math = "p_i^{(t+1)} = p_i^{(t)} - \eps \cdot \ell_i^{(t)}"/>

This algorithm has some problems -- the probabilities can become negative and they don't sum to one. These aren't so crucial; we can always round negative numbers to zero and normalize the distribution afterwards. But even after that, the algorithm doesn't feel quite right. It doesn't distinguish between changing $50\%$ to $49\%$ and changing $1\%$ to $0\%$. We're back to the issue that probability should often be handled "multiplicatively", not "additively".

Hence the multiplicative weights update algorithm:

<Math displayMode={true} math = "p_i^{(t+1)} \propto p_i^{(t)} \cdot e^{- \eps \cdot \ell_i^{(t)}}" />

The $\propto$ just means we normalize $p^{(t+1)}$ after the multiplicative update. This turns out to be a much better way to approach the problem.

## Intuitions

Let me give you three intuitions for why multiplicative weights update makes sense.

### Bayes updating

You can think about our problem as "trying to find the best expert". In this sense, the algorithm's probability distribution over experts is its "guess about who the best expert is". Whenever we learn how well each expert performed each day, this corresponds to learning "the likelihood of each expert being the best". Bayes' rule says our guess should be updated multiplicatively by multiplying it by the likelihood -- which is exactly what the algorithm does.
...
Let's think about all the information we learn in the first $t$ steps of our investing game. Each expert $i$ accumulated a total loss $L_i^{(t)} = \ell_i^{(1)} + \dots + \ell_i^{(t)}$. How should this total loss translate to the probability that we sample them in the $t+1$-th round?

We understand this! In the chapter on [max entropy distributions](04-max_entropy), we've seen how the softmax function is the right generic way of converting numbers to probabilities. So we should have <Math math = "p_i^{(t+1)} \propto e^{\lambda \cdot L_i^{(t)}}" />. The constant $\lambda$ is a proportionality constant we have to decide on by some other means.

But look -- this is exactly what the multiplicative weights algorithm is doing. It updates $p_i$ by multiplying it by <Math math = "e^{\varepsilon \cdot \ell_i^{(t)}}"/> in each step, so $p_i^{(t)}$ is proportional to <Math math = "e^{\varepsilon \cdot \ell_i^{(1)}} \cdot \dots \cdot e^{\varepsilon \cdot \ell_i^{(t)}} = e^{\varepsilon \cdot L_i^{(t)}}"/>. So what we call $\lambda$ in softmax is called the learning rate $\varepsilon$ in multiplicative weights.
...
In our optimization problem, we start with this data: the current distribution $p^{(t)}$ and loss function $\ell^{(t)}$. Notice how we try to balance two things:

1. We want the new probability distribution $p^{(t+1)}$ to be close to the original one $p^{(t)}$, whatever "close" means.

2. If we sample a random expert from $p^{(t+1)}$, we want them to be as good as possible for the current loss function. That is, we want to minimize $\sum_{i = 1}^n p^{(t+1)}_i \cdot \ell_i$.

With this in mind, here's an equivalent formulation of gradient descent and multiplicative weights:

<Math displayMode={true} math = "p^{(t+1)}_{\text{gradient descent}} = \argmin_{p} \left(\sum_{i = 1}^n \ell_i p_i + \eps \cdot \frac12 \sum_{i = 1}^n (p_i - p_i^{(t)})^2\right)."/>

<Math displayMode={true} math = "p^{(t+1)}_{\text{multiplicative weights}} = \argmin_{p, \sum_{i=1}^n p_i = 1} \left(\sum_{i = 1}^n \ell_i p_i + \eps \cdot \frac12 \sum_{i = 1}^n p_i \cdot \log \frac{p_i}{p_i^{(t)}} \right)."/>

I'll leave it to you to check that the solution of the first optimization problem is $p_{i}^{(t+1)} = p_i^{(t)} - \eps \cdot \ell_i^{(t)}$<Footnote>Hint: If you differentiate the loss function with respect to $p_i$, you get <Math math = "\frac{\partial \mathcal{L}}{\partial p_i} = \ell_i + \eps \cdot (p_i - p_i^{(t)})" />.</Footnote>, while the solution to the second optimization problem is <Math math = "p_{i}^{(t+1)} \propto p_i^{(t)} \cdot e^{-\eps \cdot \ell_i^{(t)}}" />.<Footnote>Hint: You can use Lagrange multipliers to replace the hard constraint of $\sum_{i = 1}^n p_i = 1$ with the soft constraint of $\lambda (\sum_{i = 1}^n p_i - 1)$ that goes into the loss function. After differentiating with respect to $p_i$, you get <Math math = "\frac{\partial \mathcal{L}}{\partial p_i} = \ell_i + \eps \cdot ( \log \frac{p_i}{p_i^{(t)}} + 1) + \lambda" />.</Footnote>
...
But in the context of our getting-rich riddle, we're optimizing probabilities -- after all, even gradient descent would have to be altered to keep projecting its solutions to the probability simplex to make any sense. In this space, the most meaningful way of measuring distances isn't Euclidean distance, but KL divergence. Hence, multiplicative weights.

But wait a minute! KL can't measure distance. We've emphasized how it's important that $D(p,q) \not= D(q,p)$, which seems pretty bad for measuring distance! Fortunately, we're talking about a setup where both distributions $p^{(t)}, p^{(t+1)}$ are going to end up very close to each other, and in that case, $D(p,q) \approx D(q,p)$ (more about that in the [Fisher information section](07-fisher_info)!).

## Guarantees & Applications

The multiplicative weights update algorithm has some pretty strong mathematical guarantees. In essence, if you choose the right size of $\eps$, then your algorithm will perform _almost as well as the best expert in hindsight_. Check out [this excellent survey](https://www.cs.princeton.edu/~arora/pubs/MWsurvey.pdf) for more math behind the algorithm and its uses in computer science.
...
Here's a problem: We're given a sorted array with $n$ numbers and want to find whether it contains $42$. What's the fastest algorithm?
...
This is called the noisy binary search problem and has a bunch of applications -- sometimes, comparing two objects is more complicated than comparing two numbers and may involve doing some kind of experiment which could fail.

...
2. If we sample a random expert from $p^{(t+1)}$, we want her to be as good as possible for the current loss function. That is, we want to minimize $\sum\_{i = 1}^n p^{(t+1)}\_i \cdot \ell_i.

Having this in mind, here's an equivalent formulation of gradient descent and multiplicative weights:

$$
p^{(t+1)}_{\text{gradient descent}} = \argmin_{p} \sum_{i = 1}^n \ell_i p_i + \eps \cdot \frac12 \sum_{i = 1}^n (p_i - p_i^{(t)})^2.
$$

$$
p^{(t+1)}_{\text{multiplicative}} = \argmin_{p, \sum_{i=1}^n p_i = 1} \sum_{i = 1}^n \ell_i p_i + \eps \cdot \frac12 \sum_{i = 1}^n p_i \cdot \log \frac{p_i}{p_i^{(t)}}.
$$

I will leave to you to check that the solution of the first optimization problem is $p_{i}^{(t+1)} = p_i^{(t)} - \eps \cdot \ell_i^{(t)}$<Footnote>Hint: If you differentiate the loss function with respect to $p_i$, you get $\frac{\partial \mathcal{L}}{\partial p_i} = \ell_i + \eps \cdot (p_i - p_i^{(t)})$. </Footnote>, whereas the solution to the second optimization problem is $p_{i}^{(t+1)} \propto p_i^{(t)} \cdot e^{-\eps \cdot \ell_i^{(t)}}$. <Footnote>Hint: You can use Lagrange multipliers to replace the hard constraint of $\sum_{i = 1}^n p_i = 1$ with the soft constraint of $ + \lambda (\sum\_{i = 1}^n p_i - 1)$ that goes to the loss function. After differentiating with respect to $p_i$, you get $\frac{\partial \mathcal{L}}{\partial p_i} = \ell_i + \eps \cdot ( \log \frac{p_i}{p_i^{(t)}} + 1) + \lambda$. </Footnote>
...
### Why multiplicative weights are important

[TODO divna sekce]

Multiplicative weights algorithm has many applications, the only issue is that all of them are at least a bit advanced, so we don't have space to go into details here.

1. In machine learning, this algorithm is super important in recommendation systems -- think of how Google search recommends webpages, Youtube recommends videos, and Facebook recommends posts. The connection is that you can think of each webpage/video/post being an expert and you want an algorithm that keeps choosing the best one based on the feedback you are getting from the users, in an online fashion. <Footnote> This is a good bit simplified. </Footnote>
2. In algorithms, this algorithm is used in situations where you want to satisfy many constraints at once (think of making all customers reasonably happy at once). There, you can think of each constraint as an expert. You keep choosing the least satisfied constraint (the angriest customer) and make it happy. Hopefully, you make everybody reasonably happy after a few steps of the multiplicative weights algorithm.
3. In game theory, there's a concept of Nash equilibrium in 2-people games. This equilibrium always exists, but how do you find it? Turns out, you can find it by running two players against each other, each one updating their distribution using multiplicative weights algorithm.
   \*/}

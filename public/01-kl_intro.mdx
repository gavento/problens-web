...
<Math displayMode={true} math='P(\textrm{fair} | \textrm{heads}) = \frac{P(\textrm{heads} | fair) \cdot P(\textrm{fair})}{P(\textrm{heads})}' />
...
<Math displayMode={true} math='\frac{P(\textrm{fair} | \textrm{heads})}{P(\textrm{biased} | \textrm{heads})} = \frac{P(\textrm{fair})}{P(\textrm{biased})} \cdot \frac{P(\textrm{heads} | fair)}{P(\textrm{heads} | biased)}' />
...
That is, to compute the posterior odds, we multiply two things. First, the prior odds $P(\textrm{fair}) : P(\textrm{biased})$. Second, the ratio $P(\textrm{heads} | fair) : P(\textrm{heads} | biased)$ of how likely heads are under each hypothesis. Probability expressions of the type $P(something happens | hypothesis)$ are called _likelihoods_, so this is also called the likelihood ratio.

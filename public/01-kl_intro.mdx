...
We'll break this down in the rest of the chapter, in a bit-by-bit fashion. 
...
## A bit better summary

Before we see what happens long-term, let's improve our calculator a bit. We'll get a bit more clarity if we take logs of everything so we can add instead of multiply. Instead of multiplying odds, we just add so-called _log-odds_ 

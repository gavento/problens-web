...
For example, in the widget below, make <Math math = "P(\textrm{☀️} AND \textrm{🚶‍♀️}) = P(\textrm{☁️} AND \textrm{🚲}) = \frac12" />. The mutual information is 1 bit. That's because if learning the value of one distribution, say transport, makes the entropy of the other distribution smaller by 1 bit - the weather distribution changes from a coin flip with entropy of 1 bit to either determined ☀️ or determined ☁️ with 0 entropy. 
...
<MutualInformationWidget showToggle={true} />

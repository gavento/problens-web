...
In this chapter, we'll break down what KL divergence is and how it's tied to Bayes' theorem. Plus, we'll solve [one of our riddles](mutual-information).
...
You've got a coin that might be rigged. To keep it simple, let's say there are only two options: it's either fair (50/50 for heads/tails) or it's biased toward tails (40% heads, 60% tails).
...
Bayes' theorem to the rescue! The usual formula is:

<Math displayMode={true} math='P(\textrm{fair} | \textrm{heads}) = \frac{P(\textrm{heads} | \textrm{fair}) \cdot P(\textrm{fair})}{P(\textrm{heads})}' />

No big deal—multiply both sides by $P(\textrm{heads})$ and you'll see both sides are just $P(\textrm{fair AND heads})$.
...
You can check these are the same formula. <Footnote>Use the first formula for both $P(\textrm{fair} | \textrm{heads})$ and $P(\textrm{biased} | \textrm{heads})$, then divide 'em. </Footnote> To get this version, think in [odds](https://en.wikipedia.org/wiki/Odds) instead of probabilities. Gamblers love this—instead of "1/3 chance of this, 2/3 chance of that," they say "odds are 1:2." Odds don't need to add to one (1:2 is the same as 2:4), which is handy when you don't wanna mess with normalizing.
...
Here's the picture: <Footnote>Getting Bayes' rule is super important. If this isn't clicking, check out [this explainer](https://www.lesswrong.com/w/bayes-rule). </Footnote>
...
Every flip, she multiplies her odds by a likelihood ratio: $5/4$ for heads, $5/6$ for tails.
...
First, let's rewrite our table:

![Repeated application of Bayes' theorem in log scale](01-kl_intro/repeated_bayes_log2.png)

We just wrote everything as powers of 2, so multiplying becomes adding in the exponent.

Let's clean it up even more:

![Surprisal](01-kl_intro/repeated_bayes_surprisals.png)

Instead of probability $p$ in each cell, we write $\log 1/p$. This $\log 1/p$ thing is called _[surprisal](https://en.wikipedia.org/wiki/Information_content)_. Makes sense—it measures how surprised you are when something happens. Getting heads when you thought it was 1% likely? Super surprising ($\log 1/0.01 \approx 6.6$). Getting heads on a fair coin? Meh ($\log 1/0.5 = 1$).

Our table is now like a scorecard for surprisals.
When you subtract surprisals in the same row, you get how much evidence you gained for each hypothesis. Here, heads give you 0.32 bits of evidence for fair, tails give 0.26 bits for biased.
...
We started with one bit favoring fair, so we end up with 0.85 bits for fair. Convert that back and you get 64% probability it's fair.

## Expected Distinguishing Evidence <a id="expected-distinguishing-evidence"></a>

Let's say the coin actually is biased. How fast will our Bayesian hero figure this out? We can calculate the average bits learned per flip. Heads give -0.32 bits (negative 'cause it points the wrong way), tails give +0.26 bits. On average, each flip gives:

$$
0.4 \cdot (-0.32) + 0.6 \cdot 0.26 \approx 0.03
$$

bits of evidence toward the truth. Boom—that's the KL divergence between the true 40%/60% distribution and the model 50%/50% distribution!

What's this mean in practice? You need about 33 flips to get one bit of evidence. So if you start thinking 2:1 the coin is fair, after ~33 flips you'll be at 1:1. Another 33 flips gets you to 2:1 it's biased, then 4:1, and so on.

The actual odds bounce around this average. But thanks to the law of large numbers, after $N$ flips the total evidence will be close to $0.03 \cdot N$. <Footnote> More precisely, it's $0.03N \pm O(\sqrt{N})$. We use logs and talk about bits because the law of large numbers works for adding stuff, not multiplying. </Footnote>

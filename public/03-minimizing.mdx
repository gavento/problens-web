...
<Math displayMode={true} math = {"\hat\mu, \hat\sigma^2 \
...
Setting <Math displayMode={false} math = {"\frac{\partial \mathcal{L}}{\partial \sigma} = 0$ then leads to $\hat\sigma = \frac{1}{N} \sum_{i = 1}^N (X_i - \mu)^2$"}/>

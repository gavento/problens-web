...
I claim that among all distributions <Math math="p" /> with average of 15, I should pick the distribution <Math math = "p_{\textrm{new}}" /> with 
<Math displayMode={true} math="\arg\min_{p \in P} D(p,q)," /> 
where <Math math="q" /> is my original model (the uniform distribution).

Why? Intuitively, I want to find a (true) distribution <Math math="p" /> so that <Math math="q" /> is as good model of it as possible. This is what we find by minimizing KL divergence. 

Admittedly, this reasoning is a bit shaky, since in KL, we would always like $p$ to stand for the true distribution, while here it's just a new, hopefully better model distribution. Expand for a detailed derivation from Bayes' rule.
...
Here is how we formulate our setup to turn this into an application of Bayes' theorem. Imagine that we start with some distribution $q$ and keep sampling from it $N$ times. We use $\hat{q}$ to denote the empirical distribution of the samples. Now we observe that $\hat{q}$ belongs to some family of distributions $P$ (e.g., all distributions with average 15). What is our current knowledge about $\hat{q}$?
...
We've already derived KL divergence and its application from the philosophy of using Bayes' rule. This relationship goes both ways; let's see how. <Footnote></Footnote>

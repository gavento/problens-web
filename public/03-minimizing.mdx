...
$$
\hat\mu, \hat\sigma^2 
= \argmax_{\mu, \sigma^2} \sum_{i = 1}^N \log\left( \frac{1}{2\pi\sigma^2} e^{-\frac{(X_i-\mu)^2}{\sigma^2}} \right)
= \argmin_{\mu, \sigma^2} 2N \cdot \log \sigma + \sum_{i = 1}^N \frac{(X_i-\mu)^2}{\sigma^2}
$$
...
$$
Setting $\frac{\partial \mathcal{L}}{\partial \mu} = 0$ then leads to $\hat\mu = \frac{1}{N} \sum_{i = 1}^N X_i$. 

Similarly, 
$$
\frac{\partial \mathcal{L}}{\partial \sigma} = 2N/\sigma -2  \sum_{i = 1}^N \frac{(X_i-\mu)^2}{\sigma^3}
$$
Setting $\frac{\partial \mathcal{L}}{\partial \sigma} = 0$ then leads to $\hat\sigma = \frac{1}{N} \sum_{i = 1}^N (X_i - \mu)^2$. 
...
Given this new information, how should I choose a better model? I claim that among all distributions $p$ with average of 15, I should pick $\arg\min_\{p \in P} D(p,q)$, where $q$ is my original model (the uniform distribution).

Why? Intuitively, I want to find a (true) distribution $p$ so that $q$ is as good model of it as possible, and this is what KL stands for. Admittedly, this time it's not clear why this should be the right thing to do, so see the footnote for a detailed derivation of this from Bayes' rule. 
<Footnote>
Remember how at the top of this chapter, we were able to argue that $\min_\{q \in Q} D(p,q)$ is a good methodology by "compiling it down" to a simple argument about Bayes' theorem? We will do the same here, using a version of so-called [Wallis derivation](https://en.wikipedia.org/wiki/Principle_of_maximum_entropy#The_Wallis_derivation). The reasoning is going to be a bit more involved. 
...
1. We can write it down exactly ($P(\hat{q} = p) = \{N \choose p_1N, \dots, p_kN}\cdot \prod_\{i = 1}^k q_i ^ \{p_i N}$) and solve it (using Stirling approximation, we have $P(\hat{q} = p) \approx 2^\{H(p)\cdot N}$ and thus $P(\hat{q} = p) \approx 2^\{-D(p,q)\cdot N}$). 

2. If you like to see what's happening behind the algebra, we can also go back to the [first chapter](01-kl_intro) where our Bayesian friend was pretty much trying to compute the probability we now need. More concretely, if our friend got $N$ and the empirical distribution was $p$, all the evidence in a column of a hypothesis $q$ summed up to the value $N \cdot H(p, q)$. The meaning of the evidence is that the probability $P(\hat{q} = p)$ has to be proportional to $2^\{-N \cdot H(p, q)}$, we just don't know the normalization constant.

Now, waving our hands a little, we can observe that the probability distribution of $\hat{q}$ is going to be concentrated on the distribution $p$ that minimizes $H(p,q)$, as $N$ goes to infinity. Thus, the right normalization is $P(\hat{q} = p) \approx 2^\{-N \cdot H(p,q) + N \codt H(p)} = 2^\{-N \cdot D(p,q)}$.

Whatever road you preferred, we can now continue. While the distribution of $\hat{q}$ is concentrated around the value of $q$, there's nonzero probability that $\hat{q} \in P$. If that happens, we similarly have that the posterior distribution is concentrated around the value $\tilde{p} = \argmin_\{p \in P} D(p,q)$. We conclude that in the limit of $N$ going to infinity, doing all the abstract mixture distributions and conditioning on the new evidence is pretty much equivalent to just replacing $q$ with $\argmin_\{p \in P} D(p,q)$. 
</Footnote>
...
$$D(p,q_\{uniform}) = \sum_\{i = 1}^k p_i \log \frac\{p_i \}\{ 1/k}$$

Using our splitting of KL divergence into entropy and cross-entropy, we can rewrite this as:

$$D(p,q_\text{uniform}) = \sum_\{i = 1}^k p_i\log p_i - \sum_i p_i\log (1/k)$$
...
$$\argmin_\{p \in P} D(p, q_\{uniform}) = \argmax_\{p\in P} H(p)$$
...
We will spend the whole next chapter discussing all the applications of the maximum entropy principle, but let's already make a few observations about it.

1. For discrete distributions over $\{1, \dots, k\}$, the distribution that maximizes the entropy is the uniform distribution and its entropy is $\log_2 k$. So if our set $P$ contains the uniform distribution, maximum entropy principle reduces to the principle of indifference.

2. One of the basic facts about entropy is that if you have a joint distribution $(p,q)$, then $H((p,q)) \le H(p) + H(q)$, with equality if and only if $p$ and $q$ are independent (see []()). Here's how we can use this fact: Imagine that $P$ is the set of all joint distributions with marginals $p, q$. Then, the maximum entropy principle tells us that the joint distribution $p \in P$ that maximizes the entropy is the one that is independent. 

In the practice of modeling stuff by probability distributions, it's ubiquitous to assume that two distributions are independent if we do not have any concrete guess on how they are related. We can think of this as a special case of the maximum entropy principle. 

3. Let's see one more application of the more general principle of minimizing KL divergence. 

Here's the generic problem solved by Bayes' theorem. We have a joint distribution $(X,Y)$ over two random variables $X$ (that we are interested in) and $Y$ (possible evidences we might see). Then we observe some evidence $Y=y$. We want to know the distribution of $X$ given $Y=y$. The Bayes' rule allows us to compute the conditional distribution $P(X|Y=y)$. This is not rocket science: If you imagine the original joint distribution as a table, you can just zero out the rows that do not correspond to $Y=y$ and normalize the remaining row to get the posterior distribution. <Footnote>The reason why Bayes' rule is a more complicated formula than that is that Bayes' rule computes this stuff from conditional probabilities instead of working with the joint distribution. </Footnote>

![]()

The observation I want to make is that $r = \argmin_\{p \in P} D(p, (X,Y))$, where $P$ is the set of distributions where $P(Y = y') = 0$ for $y' \ne y$. <Footnote>This holds because of the following reasoning: </Footnote> 

...
Why does this methodology make sense? Remember, KL is supposed to measure how well the model $q$ approximates the data $p$, so minimizing KL really just means picking the best fitting model in $Q$. If you still remember the [first chapter](01-kl_intro), you may even "compile" this argument down to Bayes' theorem: choosing $q$ to minimize $D(p,q)$ really means choosing $q$ that takes the longest time to distinguish from the truth $p$ by a Bayesian. Very reasonable!

### Maximum Likelihood principle <a id="mle"></a>

Let's take a closer look at our approach of minimizing $D(p, q)$. Let's focus on the most common case, where $p$ is literally the empirical distribution over $X_1, \dots, X_n$. To avoid cluttered notation, let's assume all $X_i$ are distinct<Footnote>The general case is very similar. </Footnote>. In that case, we can write the KL divergence we are minimizing as:
...
There are several ways to solve this optimization problem. The cleanest one is probably differentiation: If we define $\mathcal{L}$ to be the above expression, then:
...
Let's say I want a probability distribution for how long it takes me to eat lunch. Maybe my first (very crappy) model of this is a uniform distribution $q$ between <Math math="0" /> and <Math math="60" /> minutes.
...
Given this new information, how should I choose a better model? 

![max-ent](03-minimizing/max_ent_example.png)

I claim that among all distributions <Math math="p" /> with average of 15, I should pick the distribution <Math math = "p_{\textrm{new}} = \arg\min_{p: E_p[X]=15} D(p,q)"/> where <Math math="q" /> is my original model (the uniform distribution). In general, if I have a model distribution $q$ and learn that the true distribution is in some set $P$ (e.g. the set of distributions with mean 15), I claim that I should replace the model $q$ by a new model <Math math = "p_{\textrm{new}}"/>:

<Math displayMode={true} math="p_{\textrm{new}} = \arg\min_{p \in P} D(p,q)," /> 

Why? Intuitively, I want to find a distribution <Math math="p \in P" /> so that <Math math="q" /> is as good model of it as possible. This is what we find by minimizing KL divergence. 

Admittedly, this reasoning is a bit shaky, since when using KL divergence, we typically like $p$ to stand for the "true" distribution, while here it's just a new, hopefully better model.  

But we can also argue that this approach makes sense directly from Bayes' rule. Remember how we discussed above that using maximum likelihood really means "choose a model $q$ that's hardest to distinguish from $p$ by Bayes' rule"? Equation ?? can be similarly justified as "update $q$ to account for new evidence using Bayes' rule". 
Reasoning behind this intuition is a bit more involved and so left in an expand box. 

<Expand headline="Detailed discussion of how this follows from Bayes' rule">
The following argument is a version of so-called [Wallis derivation](https://en.wikipedia.org/wiki/Principle_of_maximum_entropy#The_Wallis_derivation) of the maximum entropy principle. 

We reformulate our setup to view ?? as an application of Bayes' theorem. 

Imagine that we start with some distribution $q$ and keep sampling from it $N$ times. We use $\hat{q}$ to denote the empirical distribution of the samples. Now we observe that $\hat{q}$ belongs to some family of distributions $P$ (e.g., all distributions with average 15). What is our current knowledge about $\hat{q}$?
...
Fortunately, there's usually a very natural choice for the simplest possible model $q$ -- the uniform distribution. This is the unique distribution that assigns the same probability to each outcome. In some sense, it is the most "uninformative" distribution we can have. This argument about uniform distribution is called the [principle of indifference](https://en.wikipedia.org/wiki/Principle_of_indifference). 

So, it would be very interesting to understand what happens when we start with the uniform distribution $q$ (let's say over a discrete set $\{1, \dots, k\}$) and find a better model <Math math = "p_{new} \in P" /> by minimizing KL divergence. Let's write it down:

<Math displayMode={true} math="p_{new} = \argmin_{p \in P} D(p,q_{uniform}) = \sum_{i = 1}^k p_i \log \frac{p_i}{ 1/k}" />

After splitting KL divergence into entropy and cross-entropy, we rewrite this as:

<Math displayMode={true} math="D(p,q_\text{uniform}) = \sum_{i = 1}^k p_i\log p_i - \sum_i p_i\log (1/k)" />

Since the second term is a constant (and in fact equal to $-\log (1/k) = \log(k)$), minimizing KL divergence is equivalent to maximizing minus the first term, which is the entropy of $p$:

<Math displayMode={true} math="p_{new} = \argmin_{p \in P} D(p, q_{uniform}) = \argmax_{p\in P} H(p)" />

We derived what's known as the [maximum entropy principle](https://en.wikipedia.org/wiki/Principle_of_maximum_entropy): given a set $P$ of distributions to choose from, we should opt for the $p\in P$ that maximizes the entropy $H(p)$.
...
One more advanced point. We have already seen that KL divergence is very closely related to the philosophy of updating your beliefs by Bayes' theorem. This relatioship actually goes both ways; more in the Expand box.

<Expand headline = "Relation with Bayes goes both ways">
We've already derived KL divergence and why it makes sense to minimize it from the philosophy of using Bayes' rule. This relationship goes both ways; let's see how. 

Consider the generic problem solved by Bayes' theorem: We have a joint distribution  <Math math = "p_{old}" /> over two random variables $X$ (that we are interested in) and $Y$ (possible evidences we might see). 
Then we observe some evidence $Y=y$. We want to compute the new distribution <Math math = "p_{new}" /> that takes this into account. The Bayes' rule is just a formula, but taken more philosophically, it's about understand what <Math math = "p_{new}" /> should be: It says that <Math math = "p_{new}" /> should be $0$ for $Y \not= y$ and for $Y = y$ it should be the conditional distribution of $p$ for $Y = y$. 

The observation we can now make is that <Math math="p_{new} = \argmin_{p \in P} D(p, p_{old})" />, where $P$ is the set of distributions where $P(Y = y') = 0$ for $y' \ne y$<Footnote>The proof of the observation requires some rewritings but in essence, it's about the fact that the distribution closest in KL to $p$ conditioned on $Y = y$ is ... the same distribution. Really, nothing complicated is happening here, we are just saying that the new distribution should look like the old one, only rescaled so that probabilities sum up to one. </Footnote>

So, we can think of the philosophy of updating by Bayes' theorem as a special case of the philosophy of minimizing KL divergence. This is all very circular and fuzzy, but the point is that minimizing KL is basically Bayes on steroids. 

{/*not only the philosophy of minimize-KL-whenever-you-can-ism can be derived from the philosophy of argue-everything-from-Bayes-rule-ism, but we can also derive the philosophy of argue-everything-from-Bayes-rule-ism from the philosophy of minimize-KL-whenever-you-can-ism. Thus it is said that minimize-KL-whenever-you-can-ism is argue-everything-from-Bayes-rule-ism-complete (and vice versa).*/}
</Expand>
...
In the [next chapter](04-max_entropy), we'll explore more concrete modeling applications of these principles. We'll see how they apply to various fields like machine learning, statistical modeling, and information theory.

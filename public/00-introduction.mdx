...
<MultipleChoiceQuestion
  ...
  correctIndices={[1, 2]}
  feedbackType="all-show"
  explanation={<>This is frankly kind of hard to say, so try it yourself below. I fitted the data by two curves - Gaussian distribution (looks like the shape in C), and Laplace distribution (looks like the shape in B). 

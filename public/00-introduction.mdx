...
<MultipleChoiceQuestion
  ...
  correctIndices={[2]}
  explanation={<>The first two work okay, but everyone uses KL divergence. Here's why it's cool: if <Math math="p_i = 0.5" /> and <Math math="q_i = 0.51" />, KL thinks that's basically fine. But if <Math math="p_i = 0.01" /> and <Math math="q_i = 0.02" />, KL freaks out (the ratios <Math math="p_i/q_i" /> are totally different). Makes sense, right?</>}
/>
...
<MultipleChoiceQuestion
  ...
  correctIndices={[]}
  feedbackType="all-show"
  explanation="Hard to tell with just 8 questions. Later, we'll see how KL divergence gives us the log-score that forecasting tournaments use."
/> 
</Expand>
...
<MultipleChoiceQuestion
  ...
  correctIndices={[2]}
  explanation={<>It's about 1,000,000! That's a huge chunk of the whole US! The general rule: to get error margin <Math math="\epsilon" />, you need roughly <Math math="1/\epsilon^2" /> samples. That square is killer—it's why getting better estimates gets expensive fast! No wonder most polls stick to a few thousand people.</>}
/>

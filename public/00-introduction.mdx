...
This site contains an introduction to various topics at the intersection of probability, information theory, and machine learning. Our tour guide through this corner of the world is a formula called _Kullback-Leibler (KL) divergence_. 

Check out [this page](about) for logistics. Here, we've got several riddles. I hope that some of them will [nerd-snipe](https://xkcd.com/356/) you! 😉 You will understand all of them at the end of this minicourse. 
...
Test your intelligence with the following widget! You will be given a bunch of text snippets cut from Wikipedia.<Footnote>It's taken from the first 1GB of big articles - so be prepared for a lot of Abraham Lincoln, Anarchy, Achilles, and spoilers from Atlas Shrugged. </Footnote> They consist of two consecutive sentences, second one being cut at random place. Your task is to predict the next letter. You can compare your performance with some leading neural nets! 

<LetterPredictionWidget/>

Why are neural nets so good in this? Why should it measure intelligence? And why did Claude Shannon - the information theory GOAT - make this experiment in 1940's? 
...
<MultipleChoiceQuestion
  ...
  What's happening here? We will discuss this example in the <a href="04-max_entropy">chapter about the max-entropy principle</a>. This principle is extremely useful in situations like ours, when we want to model some kind of data. <br/><br/></>}
/>
...
This question is for those of you who already who already encountered KL divergence and entropy. 
...
<Expand advanced={true} headline="🗳️ Why polling sucks"><a id="polling"></a>
...
<Expand advanced={true} headline="💰 How to get rich"> <a id="get-rich"></a>
...
As we go through the mini-course, we'll use KL divergence to revisit each puzzle and understand what's going on. 

There are five chapters. Here's what they cover. 

1. We learn what KL divergence stands for and how it's connected to Bayes' rule. 
2. We meet its friends cross-entropy and entropy. 
3. We understand how minimizing KL divergence leads to maximum likelihood and maximum entropy principles. 
4. We cover the max entropy principle and its applications. 
5. We apply all our tools to machine learning. 

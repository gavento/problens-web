...
Answer 1: Choose small $\varepsilon$ and do gradient descent: $p^{(t+1)} = p^{(t)} - \eps \cdot \ell_i^{(t)}$; normalize $p^{(t+1)}$ afterwards.
Answer 2: Choose small $\varepsilon$ and do "multiplicative" gradient descent: $p^{(t+1)} = p^{(t)} \cdot e^{- \eps \cdot \ell_i^{(t)}}$; normalize $p^{(t+1)}$ afterwards.
Answer 3: Just follow the best guy so far: $p^{(t+1)}_i = 1$ for the guy $i$ with minimum $\ell_i^{(1)} + \ell_i^{(2)} + \dots + \ell_i^{(t)}$.

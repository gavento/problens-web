...
<MultipleChoiceQuestion
  ...
  correctIndices={[1]}
  feedbackType="all-show"
  explanation={<>I would expect the plot look like the shape C (Gaussian distribution), but it looks more like the shape B (Laplace distribution). 
  <FinancialDistributionWidget showBTC={false} showSAP={true} />
  What's up with that? We will discuss in the <a href="04-max_entropy">chapter about the max-entropy principle</a>.<br/><br/></>}
/>

...
<MultipleChoiceQuestion
  options={[
    <><Math math="\mathcal L(p,q) = \sum_{i = 1}^k |p_i - q_i|" /> (<em><Math math="\ell_1" /> norm</em>)</>,
    <><Math math="\mathcal L(p,q) = \sum_{i = 1}^k (p_i - q_i)^2" /> (<em><Math math="\ell_2" /> norm</em>)</>,
    <><Math math="\mathcal L(p,q) = \sum_{i = 1}^k p_i \cdot \log (p_i / q_i)" /> (<em>KL divergence</em>)</>
  ]}
  correctIndices={[2]}
  explanation={<>The first two work okay, but everyone uses KL divergence. Here's why it's cool: if <Math math="p_i = 0.5" /> and <Math math="q_i = 0.51" />, KL thinks that's basically fine. But if <Math math="p_i = 0.01" /> and <Math math="q_i = 0.02" />, KL freaks out (the ratios <Math math="p_i/q_i" /> are totally different). Makes sense, right?<br/><br/>Check out this widget with two distributions and all three loss functions. See if you can make KL go to infinity!<br/><br/><DistributionComparisonWidget title="KL Divergence Explorer" /></>}
/>
...
<MultipleChoiceQuestion
  ...
  correctIndices={[0, 1, 2]}
  feedbackType="all-show"
  explanation="Hard to tell with just 6 questions. But I am not a big fan of 🧑🏻 though, with his failed 99%-confidence prediction! Later, we'll see how KL divergence gives us the log-score that forecasting tournaments use; this score penalizes failed 99%-confidence predictions quite heavily! "
/> 
</Expand>
...
<MultipleChoiceQuestion
  ...
  correctIndices={[1]}
  explanation={<><a href="01-kl_intro/information-theory">We'll see</a> this is measured by something called mutual information, which is super important in information theory and—surprise!—it's just KL divergence in disguise.</>}
/>
</Expand>
...
<MultipleChoiceQuestion
  ...
  correctIndices={[2]}
  explanation={<>It's about 1,000,000! That's a huge chunk of the whole US! The general rule: to get error margin <Math math="\epsilon" />, you need roughly <Math math="1/\epsilon^2" /> samples. That square is killer—it's why getting better estimates gets expensive fast! No wonder most polls stick to a few thousand people.<br/><br/>But why <Math math="1/\epsilon^2" />? We'll dig into this later.</>}
/>
</Expand>
...
<MultipleChoiceQuestion
  options={["Gradient descent", "Multiplicative gradient descent", "Follow the leader"]}
  correctIndices={[1]}
  explanation={<>They all seem reasonable, but option 2 is the most robust. It's called multiplicative weights update, and it looks weirdly like gradient descent. <a href="06-algorithms">We'll see</a> how KL divergence explains the connection.</>}
/>
</Expand>
...
<MultipleChoiceQuestion
  ...
  correctIndices={[1, 2]}
  explanation={<>We'll use KL divergence to derive something called the <a href="04-max_entropy/xkcd">maximum entropy principle</a> that explains why our gut feeling says we're probably screwed here.</>}
/> 
</Expand>
...
<MultipleChoiceQuestion
  ...
  correctIndices={[2]}
  explanation={<>It's around 100MB. But wait—why does an <em>AI</em> researcher care about compression? We'll see the connection with <a href="02-crossentropy/coding">entropy</a> and <a href="#">Kolmogorov complexity</a>.</>}
/> 

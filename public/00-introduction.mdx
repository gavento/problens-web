...
<MultipleChoiceQuestion
  options={[
    <>Gradient descent: <Math math="p^{(t+1)} = p^{(t)} - \varepsilon \cdot \ell_i^{(t)}" />, then normalize</>,
    <>Multiplicative gradient descent: <Math math="p^{(t+1)} = p^{(t)} \cdot e^{-\varepsilon \cdot \ell_i^{(t)}}" />, then normalize</>, 
    "Follow the leader: Trust whoever has smallest total loss so far"
  ]}
  correctIndices={[1]}

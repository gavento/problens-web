...
|-------------|-------------|------------------------|-----------------|
| **Surprisal** | $\log \frac{1}{p}$ | Surprise of an outcome | Bits to store a letter |
| **Entropy** | $ \sum p_i \log \frac{1}{p_i}$ | Expected surprisal | Average bits per letter |
| **Cross-entropy** | $\sum p_i \log \frac{1}{q_i}$ | Expected surprisal of model $q$ | Average bits per letter for code optimized for $q$ |
| **KL divergence** | $\sum p_i \log \frac{p_i}{q_i}$ | Additional surprise from $p \neq q$ | Additional storage from $p \neq q$ |


## 📦 Adaptive compression and LLMs
...
Let's discuss two compression algorithms, the first one hypothetical, the second one widely used. 
...
We have discussed how LLMs are trained by minimizing the cross-entropy loss:
<Math math = ""/>


in the main text; they are trained by reading the Internet, minimizing the cross-entropy loss. But we can now understand that equivalently, __they are trained to compress Internet as much as possible__!  
...
### Really, prediction = compression
Here is the generic approach for how most compressions algorithm work that I'll call adaptive compression. An adaptive compression algorithm goes through the text letter by letter and always tries to use the past text to predict the next letter. Knowing the next letter can be turned into efficient compression.   If it is doing good job predicting, it's also doing a good job compressing. 

As we know, once you have a distribution over the next letter, we can convert it to a code, e.g. Shannon's code. This way, we can encode the whole text.  

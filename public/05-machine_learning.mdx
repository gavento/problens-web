...
<Math displayMode={true} math = "q(x | y) \prop e^{-\| x - Dec(y)\|^2}{2d}"/> 
and hence, up to additive constant that does not play a role in our optimization problem, we have
<Math displayMode={true} math = "\log q(x | y) = -frac{\| x - Dec(y)\|^2}{2d}"/> 

So, if we were in the setup of a regular autoencoder where $p'(y|x)$ is a deterministic encoding $Enc(x)$, minimizing the term $\mathcal L_1$ would boil down to minimizing the mean square _reconstruction_ loss 
<Math displayMode={true} math = "\sum_{i = 1}^N frac{\| X_i - Dec(Enc(X_i))\|^2}{2d}" />. 

Our case is more complicated, because the loss function involves $p'(y|x)$ that samples from <Math math = "N(Enc_\mu(x), Enc_{\sigma^2}(x))" /> instead of being a deterministic encoding. We will discuss in the end how to estimate this part of the loss function. 
...
<Math displayMode={true} math = "\sum_{i = 1}^N \frac{1}{N} \sum_y p'(y|x) \log \frac{p'(y|x)}{q(y)} = \sum_{i = 1}^N \frac{1}{N} D(p'(y|x), q(y))"/>
...
The first distribution $p'(y |X_i)$ is simply a gaussian with mean $Enc_\mu(X_i)$ and and variance being the diagonal matrix with entries <Math math = "Enc_{\sigma^2}(X_i)" />. The second distribution $q(y)$ is literally the gaussian $N(0,1)$. Now, it is a [fact](https://leenashekhar.github.io/2019-01-30-KL-Divergence/) that there's a simple formula for the KL divergence between two gaussians. It looks like this:
<Math math = "D(N(\mu, \sigma^2), N(0, 1)) = \frac12 \sum_{i = 1}^d \mu_i^2 + \sigma_i^2 - 1 - \log\sigma_i^2"/>

Plugging it to $\mathcal L_2$, we get that we have to minimize the expression 
<Math displayMode = {true} math = "\frac{1}{N} \sum_{i = 1}^N \left( \frac12 \sum_{j = 1}^d Enc_{\mu, j}(X_i)^2 + Enc_{\sigma^2, j}(X_i) - \log Enc_{\sigma^2, j} \right)"/>

</Expand>

After we do the algebra, we end up with the loss function
<Math displayMode={true} math = "\frac{1}{N} \sum_{i = 1}^N \left( \sum_y p'(y | x) frac{\| X_i - Dec(y)\|^2}{2d} \,+\, \left( \frac12 \sum_{j = 1}^d Enc_{\mu, j}(X_i)^2 + Enc_{\sigma^2, j}(X_i) - \log Enc_{\sigma^2, j} \right)\right)"/>

In general, computing the expression <Math math = "\sum_{i = 1}^N \sum_y p'(y | x) frac{\| X_i - Dec(y)\|^2}{2d}"/> is incredibly hard since we would have to iterate overall the whole range of $p'(y|x)$. In practice, this expression can be estimated well-enough by the Monte Carlo method. That is, in practice we just sample a few times from $p'(y|x)$ and compute the reconstruction loss $\| X_i - Dec(y)\|^2$ for them. We also add a few more regularization terms that make sure that the our variational autoencoder does not set $\sigma_i^2 = 0$ to become a vanilla autoencoder. 

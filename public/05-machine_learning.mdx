...
First, we transform the general idea that mean and variance are important into a concrete probabilistic model. The **maximum entropy principle**, introduced in [the previous chapter](04-max_entropy), suggests modeling the data as [independent samples](03-minimizing/a-few-observations) drawn from [the Normal distribution](04-max_entropy/normal).

Once we have a set of possible models—all Gaussian distributions—we can select the best one using [the maximum likelihood principle](04-minimizing/mle).<Footnote>In the equations, it will be easier to maximize the logarithm of the likelihood function, i.e., the log-likelihood. This is perhaps not too surprising since [we understand](03-minimizing/mle) that maximizing log-likelihood is fundamentally equivalent to minimizing KL divergence, which is indeed what's happening here.</Footnote> As [we did in an earlier chapter](04-minimizing/mle_for_mean_sigma), the outcome is that we framed the problem of finding $\hat\mu_{MLE}, \hat\sigma^2_{MLE}$ as minimizing the following *loss function*:
...
In this specific case, the optimization problem yields closed-form solutions: $\hat\mu_{MLE} = \frac{1}{N} \cdot \sum_{i = 1}^N X_i$ and $\hat\sigma^2_{MLE} = \frac{1}{N} \cdot \sum_{i = 1}^N (X_i-\hat\mu_{MLE})^2$. Thus, we arrived at explicit formulas for these crucial parameters. While the formulas themselves don't explicitly mention probabilities, probabilities are essential for understanding the underlying mechanics.

What I want to emphasize is how our only initial assumption about the data was simply, "we have a bunch of numbers, and we care about their mean and variance." The rest flowed automatically from our understanding of KL divergence. Notably, we didn't need to invoke the Central Limit Theorem at any point.
...
Now, let's proceed with the third step of our recipe: applying the **maximum likelihood principle**. We write down the likelihood of our data given our model:
...
As is customary, it's simpler to consider the log-likelihood (or cross-entropy):
...
Therefore, we can effectively disregard the added parameter $\sigma^2$<Footnote>Statistical software like R or Python's statsmodels also outputs $\sigma^2$. This can be helpful if you also, given some new point $x_{n+1}$, want not just to predict $y_{n+1}$, but also its error bars.</Footnote> and simply optimize this expression, which is the classical [least-squares loss function](https://en.wikipedia.org/wiki/Ordinary_least_squares) for linear regression.<Footnote>Typically, more variables $x^1, \dots, x^k$ are used to predict $y$. This leads to more complex formulas, but the derivation remains the same.</Footnote> Observe how the **square in the loss function emerged directly from our maximum entropy assumption of Gaussian noise**.
...
This time, we're presented with red and blue points on a plane, and our goal is to find the optimal line that separates them. Ideally, all red points would be on one side and all blue points on the other, but this isn't always achievable. In such cases, how do we determine the "best" separating line?

It will be more beneficial to represent the line not as $y = ax+b$, but using its normal vector $\theta$ (orthogonal to the line) and the distance $\delta$ of the origin from the line, as depicted in the image below.<Footnote>Specifically, $\theta = \frac{1}{a^2+1} (a, -1)$ and $\delta = |b|/\sqrt{a^2+1}$, but let's not get bogged down in those details.</Footnote>
...
Now, employing the **maximum entropy principle**, we construct a probabilistic model that transforms this distance into a probability of color. We utilize the [logistic function](04-max_entropy/logit). That is, our model states:
...
Once we have a model, we can apply the **maximum likelihood principle** to find its parameters. This principle instructs us to find $a,b,\lambda$ that minimize the following expression:
...
where $\ell_i$ is an indicator variable (i.e., $\ell_i \in \{0,1\}$) denoting whether the point $(x_i,y_i)$ is red. This problem, known as [logistic regression](https://en.wikipedia.org/wiki/Logistic_regression), is challenging to solve exactly, but gradient descent typically performs quite well.
</Expand>
...
In practice, people often aren't as concerned with this probabilistic assignment; knowing the closest cluster is usually sufficient. This corresponds to considering the limit as $\sigma \rightarrow 0$. In this limit, the formidable expression above simplifies quite elegantly. Specifically, we can replace the summation <Math math = "\sum_{j = 1}^k e^{-\frac{||x_i-\mu_j||^2}{2\sigma^2}}" /> with <Math math = "\max_{j = 1}^k e^{-\frac{||x_i-\mu_j||^2}{2\sigma^2}}" /> because all terms in the sum, except the largest one, become negligible as $\sigma \rightarrow 0$. The expression then simplifies to:
...
Designing the architecture of a neural network is a highly intricate problem that cannot be "solved" simply by applying our current tools. However, **maximum entropy** does offer some assistance: [we've already discussed](04-max-entropy/softmax) that the final layer of the network typically converts logits into probabilities using a **softmax function**. That is, if we build a neural network that transforms an input image $X$ into $k$ numbers $NN_1(X), \dots, NN_k(X)$, we should map them into probabilities as <Math math = "p_j(X) \propto e^{\lambda NN_j(X)}" />. The constant $\lambda$ can be optimized alongside the network's weights or simply hardcoded to $1$.
...
Maximizing the log-likelihood is equivalent to minimizing **cross-entropy** ([as we discussed in this chapter](03-minimizing/mle)). Therefore, in this context, we usually refer to this as "training the network by minimizing the cross-entropy loss <Math math = "\sum_{i = 1}^n \log 1/p_{\ell_i}(X_i)" />."
</Expand>
...
As the next step, we now desire a system that can not only classify images but also generate new ones from scratch (think DALL-E or Midjourney) or perform the cool trick mentioned above, where we interpolate between two images *in the latent space*. The foundational approach for this is called *autoencoders*.

An autoencoder (see below) is a neural network architecture that processes an input image $x$ through an encoder, $\textrm{Enc}(x)$, compressing it into a smaller list of numbers $y$. Subsequently, a decoder, $\textrm{Dec}(y)$, attempts to reconstruct the original image $x$. The hope is that due to the bottleneck in the middle, $y$ captures the essential aspects of image $x$ while discarding less important ones. In other words, the encoder is hoped to encode the image into the latent space.
...
Finishing the job now boils down to algebra, which is conveniently presented in the following Expand box:
...
In general, computing the expression <Math math = "\sum_{i = 1}^N \sum_y p'(y | X_i) \frac{\| X_i - \textrm{Dec}(y)\|^2}{2d}"/> is incredibly challenging, as it would require iterating over all the whole range of $p'(y|X_i)$. In practice, this expression can be adequately estimated using the Monte Carlo method.

That is, in practical applications, we can optimize this loss function. For each image $X_i$, we simply sample a few times from $p'(y|X_i)$ and compute the reconstruction loss $\| X_i - \textrm{Dec}(y)\|^2$ for these samples. We also add the regularization term involving the mean and variance output by the encoder; this ensures that our variational autoencoder doesn't degenerate into a vanilla autoencoder by setting $\sigma_i^2 = 0$.
</Expand>

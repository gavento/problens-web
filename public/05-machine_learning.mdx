...
We now have a probabilistic model parameterized by $k+1$ numbers: $\mu_1, \dots, \mu_k, \sigma^2$. We will use maximum likelihood to find those parameters. The principle tells us that we should minimize this expression:

<Math displayMode = {true} math = "\argmin_{\substack{\mu_1, \dots, \mu_k \\ \sigma^2}} \frac{n}{2\pi k} \log \sigma^2 + \sum_{i = 1}^n - \sum_{i = 1}^n \log \sum_{j = 1}^k e^{-\frac{||x_i-\mu_j||^2}{\sigma^2}}"/>

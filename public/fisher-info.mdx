...
$$I(\theta) = E_{x \sim p(x|\theta)} \left[ \left( \frac{\partial}{\partial \theta} \log p(x|\theta) \right)^2 \right]$$

This can be equivalently written as:

$$I(\theta) = -E_{x \sim p(x|\theta)} \left[ \frac{\partial^2}{\partial \theta^2} \log p(x|\theta) \right]$$
...
$$D_{KL}(p(x|\theta) || p(x|\theta + \epsilon)) = \frac{\epsilon^2}{2} I(\theta) + O(\epsilon^3)$$
...
$$\frac{\partial H(p_\theta)}{\partial \theta} = -E_{x \sim p_\theta} \left[ \frac{\partial \log p_\theta(x)}{\partial \theta} \right]$$

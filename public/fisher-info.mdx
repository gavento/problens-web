...
For a probability distribution <Math math="p(x|\theta)" /> parameterized by <Math math="\theta" />, the **Fisher information** is defined as:
...
Consider two distributions with parameters <Math math="\theta" /> and <Math math="\theta + \epsilon" />. For small <Math math="\epsilon" />:
...
Fisher information provides a fundamental limit on how well we can estimate parameters. The **Cramér-Rao bound** states that for any unbiased estimator <Math math="\hat{\theta}" /> of parameter <Math math="\theta" />:

<Math displayMode={true} math="\text{Var}(\hat{\theta}) \geq \frac{1}{n \cdot I(\theta)}" />

where <Math math="n" /> is the sample size. This tells us:
- Higher Fisher information → lower variance bound → better estimation possible
- The bound scales with <Math math="1/n" />, explaining why more data helps

## Examples

### Gaussian Distribution
For a Gaussian with unknown mean <Math math="\mu" /> and known variance <Math math="\sigma^2" />:
...
For a Bernoulli distribution with parameter <Math math="p" />:
<Math displayMode={true} math="I(p) = \frac{1}{p(1-p)}" />

Information is maximized when <Math math="p = 0.5" /> (maximum uncertainty).

...
## 🔬 Programming Language Similarity Experiment

Here's an interesting application of compression theory: we can measure how similar programming languages are by training compression algorithms on one language and seeing how well they compress another language. Languages with similar syntax, keywords, and structure should compress each other better.

<ProgrammingLanguageSimilarityWidget />

This visualization shows a network where each node represents a programming language, and the connections (edges) show compression-based similarity. The widget demonstrates three different compression approaches:

1. **Character Frequency + KL**: Uses character frequency distributions and KL divergence as a baseline similarity metric
2. **Zstd**: Uses Zstandard dictionary-based compression - train a dictionary on one language, compress another
3. **Gzip**: Uses gzip with concatenated "pseudo-dictionaries" for comparison

Languages from similar families (like C/C++/Java or Haskell/Lisp) should cluster together, while very different paradigms should be farther apart. The experiment connects information theory, compression, and programming language design!

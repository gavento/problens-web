...
For example, if we are flipping a fair coin and want to keep the results, there is nothing smarter then remembering a file like <Math math="\textsf{0101100}" /> where <Math math="\textsf{0}" />/<Math math="\textsf{1}" /> corresponds to <Math math="\mathsf{H}" />/<Math math="\mathsf{T}" />. This is 1 bit per flip. 

...
In the [previous chapter](04-mle), we saw how minimizing $q$ in the KL divergence $D(p, q)$ helps us to find the best model of the truth $p$. 

In this chapter, we also the second question - what happens if we minimize $p$ in $D(p, q)? This way, we will build the so-called _maximum entropy principle_ that guides us when we want to choose a good model distribuiton. 
...
In the previous chapter, it was the second term that was constant, now it's the first term: $\sum_{i = 1}^k p_i\log \frac{1}{1/k} = \log(k)$ does not depend on $p$, so minimizing KL divergence is, for uniform distribution, equivalent to minimizing $-H(p)$. So, we get:
...
Let's first understand the simplest possible case: when our distribution is supported on only two outcomes, $a_1, a_2$. Then, the max-entropy distribution takes the form:
...
<Expand headline = "Example: Probabilities vs. Logits">
...
<Expand headline="Example: Logistic activation function">
...
![Are the odds in our favor?](00-riddles/countdown.png)
...
Let's revisit our financial mathematics riddle [from the intro](00-riddles/financial-mathematics). In that riddle, we look at daily changes in S&P index (normalized as a so-called log-return $\log S_{i+1}/S_{i}$ for all consecutive daily values $S_i, S_{i+1}$) and plot them in a histogram in the following widget
...
In the [next chapter](07-machine_learning), we'll see how KL divergence explains the choice of loss functions used in machine learning.
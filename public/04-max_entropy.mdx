...
For example, in the previous chapter we were interested in the set $P$ of distributions where $E[X] = \mu$ and $E[(X-\mu)^2] = \sigma^2$. It turns that given these two constraints, the maximum entropy distribution is the normal distribution where <Math math = {"p(x) \propto e^{-(x-\mu)^2/\sigma^2}."} />

If we were only fixing the value of $E[X]$ (and the domain were nonnegative real numbers), the maximum entropy distribution would be the exponential distribution where <Math math = {"p(x) \propto e^{-\lambda x}"} />. Here, $\lambda$ is some parameter -- fixing $E[X]$ to have different values leads to different $\lambda$s. 
...
But I want to give you an intuition. Remember, we understand from [the previous chapter](03-minimizing) that the max-entropy principle is really about finding a distribution that is close to being uniform, in the sense of minimizing KL divergence. 
...
The answer is that both have the same probability since $p(x_1)\cdot p(x_2) = e^{-x_1 - x_2}$ and we have in our riddle that $x_1 + x_2 = x_1' + x_2'$. In other words, in this density graph, the lines with the same sum $x_1 + x_2$ have the same probability density. 
...
I am mentioning this just to make sure that  
</Footnote>

...
<Math displayMode={true} math = "p(a_1) = \frac{1}{e^{- \lambda a_1} + e^{- \lambda a_2}},\;\;\; p(a_2) = \frac{e^{-\lambda}}{e^{- \lambda a_1} + e^{- \lambda a_2}}"/>
...
Since in our example, there were just two hypotheses, we subtracted the two relevant surprisals and worked with "log-odds", a very intuitive term that says what the numbers are. In this case, we could also call the log-odds number the logit, these are basically synonyms. As far as I know, when there are more then two hypotheses, people stick with the language of using "logits".  
</Expand>

...
This kind of distribution is known under several names, depending on what is the underlying ground set over which we define it. Let's first understand it in the simplest possible case when our distribution is supported only on two possible outcomes $a_1, a_2$. Then, the max-entropy distribution takes the form of 
...
But really, it's better to write it like this:

<Math displayMode={true} math = "p(a_1) = \frac{1}{1 + e^{- \lambda (a_1 - a_2)}},\;\;\; p(a_2) = \frac{e^{-\lambda (a_1 - a_2)}}{1 + e^{- \lambda(a_1 - a_2)}}"/>

The function $\sigma(x) = 1/(1 + e^{-x})$ is called the [logistic function](https://en.wikipedia.org/wiki/Logistic_function)<Footnote>Or sometimes a sigmoid function. Apparently, [sigmoid function](https://en.wikipedia.org/wiki/Sigmoid_function) is a bit more general term. </Footnote> and the parameter $x$ is typically called a [logit](https://en.wikipedia.org/wiki/Logit). Using this function, our max-entropy distribution can be written like this:

<Math displayMode={true} math = "p(a_1) = \sigma(\lambda (a_1 - a_2)),\;\;\; p(a_2) = 1 - \sigma(\lambda (a_1 - a_2))"/>

Here's how I interpret this: if you want to talk about probabilities, but for whatever reason you want to work with general real numbers instead of numbers from $[0,1]$, you should talk about logits and use the logistic transformation to convert them to probabilities. The parameter $\lambda$ in the formula corresponds to the choice of units in which we measure those logits. For example, I usually like to use the binary logarithm, which corresponds to measuring logits in bits. 


<Block headline = "Logistic function">
The logistic function $\sigma(x) = 1/(1 + e^{-x})$ is the right function to convert between numbers and probabilities. 
</Block>
[TODO logit widget]

<Expand headline = "Elo system">
The Elo system is the system used to assign ratings to players. The basic idea of it is that when you have two players with Elos $E_1$ and $E_2$, you can estimate the probability that the first one wins from the difference $D = E_1 - E_2$. As we just discussed, the softmax function for $E_1, E_2$ is equivalent to mapping $D$ to a probability via the logistic function. 

This is indeed how Elo system was designed; the interpretation of the chess Elo number is that the probability that the first player wins should be <Math math = "\frac{1}{1 + 10^{D / 400}}" />. <Footnote>It's more precisely the expected score of the first player due to the existence of draws. </Footnote> This is the logistic function for $\lambda = 1/400 \cdot \ln 10$. 
...
When we analyzed the Bayesian hero from the [first chapter](01-kl_intro), it was easier to work with logarithms of odds, instead of odds themselves. Whenever this happens, we can say that we work "in the logit space". 

Since in our example, there were just two hypotheses, we subtracted the two relevant surprisals and worked with so-called "log-odds". We could as well call the log-odds number the logit, these are basically synonyms in a sense. As far as I know, when there are more then two hypotheses, people stick with the language of using "logits".  
</Expand>
...
Let's stay with the max-entropy distribution for $E[X]$, but now let's look at the more general case where the where our distribution is supported on a set of numbers $\{a_1, \dots, a_k\}$ -- these distributions are often called [categorical distributions](https://en.wikipedia.org/wiki/Categorical_distribution). In that case, the max entropy distribution is what's called a softmax (or softmin) distribution -- each number $a_i$ is chosen with probability 

<Math displayMode={true} math = "p_i \propto e^{\lambda a_i}. "/>

You can think about the softmax function (or softmin if $\lambda < 0$) as the generalization of the logistic function above -- it's _the right way_ to convert a bunch of numbers to probabilities. The numbers $a_i$ are still often called logits. 
...
Typical neural networks output probabilites. For example, a typical neural network classifying images in fact outputs the whole probability distribution that a given picture is a dog/cat/horse/... instead of outputting a single prediction. 
...
If the domain of the probability distribution is nonnegative integers/reals, then we get the so-called [geometric](https://en.wikipedia.org/wiki/Geometric_distribution)/[exponential](https://en.wikipedia.org/wiki/Exponential_distribution) distribution -- another very basic and useful distribution. You should think about this distribution as _the most natural_ distribution on the domain of positive numbers (either integers or reals). 
...
I think this is a good (partial) explanation of "why is there $x^2$ in Gaussian"? It simply means that Gaussian is the *right* distribution to work with if you care about the mean and the variance. If you cared about the first four [moments](https://en.wikipedia.org/wiki/Moment_(mathematics)), the right family of distributions would be those with four-degree polynomials in the exponent. 

Often, I like to think about the Gaussian as the simplest possible distribution on real numbers - Neither the uniform distribution nor the exponential distribution can be normalized there, so we salvage this by fixing the first two moments. 

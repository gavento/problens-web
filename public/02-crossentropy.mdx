...
I actually like to think about Kraft's inequality as equality. Why? Well, if your code is such that the left-hand side is really smaller than $1$, then you are stupid. You can improve the code like in the picture below and get <Math math="\sum_{i = 1}^k 2^{- \ell_i} = 1" />. 

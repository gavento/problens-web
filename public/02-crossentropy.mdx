...
In fact, that's why we call it a [_divergence_](https://en.wikipedia.org/wiki/Bregman_divergence) instead of a distance—it acts kinda like a distance but isn't symmetric.
...
Here's a riddle for you! The following widget works with two texts: Declaration of Independence and a collection of several anthems. For both texts, we compute the distribution of the frequencies of letters 'a' to 'z' (case insensitive). All probabilities in both distributions are strictly larger than zero, so the two KL divergences between the two texts are finite. The riddle is: which one is larger? 

two distributions of letter frequencies $p_1,p_2$ for two texts: English prose and a collection of anthems of various countries. Both distribution   

Try it yourself! Calculate the KL divergence between letter frequencies of different texts to see the asymmetry in action:
...
<Expand headline="Monotonicity" advanced={true}>

There's also a stronger version of nonnegativity called monotonicity. 
...
<Expand headline="Chain rule" advanced={true}>
...
For similar reasons, KL between $p_1$ and $p_2$ is smaller than vice versa. Remember, KL is all about probability _ratios_. If $p('e') = 0.12$ and $q('e') = 0.11$, $q$ is a good model of $p$. But if $p('z') = 0.02$ and $q('z')=0.0003$, $q$ is a crappy model of $p$. 

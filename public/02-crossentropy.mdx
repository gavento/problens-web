...
Splitting a summation into two parts is not some kind of hard-core mathematical trick -- the main advantage of adding cross-entropy & entropy to our vocabulary is that they are simply very meaningful concepts. <Footnote>In fact, entropy is probably a good bit more well-known concept than KL divergence. The reason why this mini-course builds everything up using KL divergence and not entropy is related to the fact that I am trying to strike a balance between an excited stream-of-consciousness wall of text about fun and underrated applications of probability, and something resembling a coherent text. </Footnote>
...
KL divergence and entropy are closely connected to coding theory. There are [many amazing explainers of that](resources) for that, so let's only see a quick intuition for why this is so closely connected.  

<Expand headline = "Coding Theory Intuitions"> <a id="coding"></a>

Suppose that we have a long string; for simplicity, let's say the alphabet of the string is A,C,G,T (the string encodes DNA). We want to store the string on the disk using the least amount of space possible.  

We will make this problem easier for ourselves by disregarding correlations between letters. Technically speaking, we will assume that there is some underlying distribution $p$ over the alphabet and our string was produced by sampling repeatedly from $p$.  

If $p$ is the uniform distribution, the best encoding is to simply use two bits per letter -- for example: <Math math = "A \rightarrow \textsf{00}, C \rightarrow \textsf{01}, G \rightarrow \textsf{10}, T \rightarrow \textsf{11} " />. This way, a string of length $N$ has size of $2N$ bits on the disk. 

But what if $p$ is skewed towards some letter? For example, what if $A$ has probability $1/2$, C has probability $1/4$, and G with T have $1/8$ both? Then there is a better encoding: <Math math = "A \rightarrow \textsf{0}, C \rightarrow \textsf{10}, G \rightarrow \textsf{110}, T \rightarrow \textsf{111} " />. On this example, this encoding uses only $1.75N$ bits on average to store the string on the disk. 

![encoding](02-crossentropy/encoding.png)

It makes sense that more frequent letters get shorter code, but what is the best way to map frequencies into code-lengths? Long story short, the coding theory says that if we want to build a good code, then the letter with frequency $p$ should be assigned a bit string of length around $\log 1/p$. For example, since $A$ had probability $1/2$ above, our optimized code assigned it a bit string of length $\log 1/(1/2) = \log 2 = 1$; and similarly for C,G,T. <Footnote>I sneakily chose an example where all probabilities are powers of two. Then, what I said about lengths of $\log 1/p$ is really true. If probabilities are general numbers, it gets more complicated because we can't have code names of noninteger length. For example, if there are five letters with the same frequency, we unfortunately can't have a code that gives each letter a code name of length $\log 5 \approx 2.3$. The good codes solve this problem by giving code names to small tuples of letters instead of just single letters; this way they approach the goal of spending $\log 1/p$ bits for a frequency-$p$ letter. </Footnote> 

Here's roughly the intuition why the formula $p \rightarrow \log 1/p$ makes sense for code lengths. First, imagine the case of uniform distribution, like 8 letters, each coming with frequency $1/8$. Then, it makes sense that the best code gives code names of same length to each letter, and the right length is $\log 8 = 3$ so that we can cover all letters. 

More generally, it makes sense to assign code names starting with <Math math = "\textsf{0}" /> to letters with overall frequency $1/2$, and the rest should start with <Math math = "\textsf{1}" />. For example, our code assigned <Math math = "\textsf{0}" /> to A and <Math math = "\textsf{1}" /> is the starting bit for codes of C,G,T. This way of repeatedly splitting the letters in halves leads to code lengths of $\log 1/p$ for letters of frequency $p$. <Footnote>Again, this works if probabilities are powers of two, if not, building good codes gets a bit more technical. </Footnote> 

The upshot is that if our string was generated by sampling $N$ letters independently from a distribution $p$, we can build a code that maps the string to a bit string with about $N \cdot \sum_{i = 1}^k p_i \cdot \log 1/p_i = N \cdot H(p)$ bits -- because the code needs to use $\log 1/p_i$ bits with probability $p_i$. It can also be proven that it's impossible to store such bit strings with less than $N \cdot H(p)$ bits of memory. 

This can give us a lot of intuition for entropy. For example, take this picture we already discussed. 

![entropy](02-crossentropy/entropy.png)

We discussed the picture using the language of entropy as "average surprisal", but really, we can also talk about how many bits are needed (on average) to store the coin-flipping outcomes to memory. On the left, we need 1 bit per outcome. On the right, it's more frugal to only store the positions of heads, which on average requires much less space than 1 bit per flip. 

We can also get intuition about cross-entropy: This is the number of bits you need if your data is coming from $p$, but you mistakenly optimize your code for a distribution $q$. Finally, relative entropy is simply how much worse your cross-entropy code is than the best possible code for $p$. 


<Expand headline = "Riddle: Size of Wikipedia">
Let's go back to [our riddle](00-introduction/wikipedia) about the size of Wikipedia and Hutter's prize to compress 1GB of it as much as possible. 

Our discussion about how to encode letters in binary is giving us one benchmark -- if we treat each letter of the text independently coming from a distribution $p$ of English letter-frequencies, we can simply use an encoding optimized for that distribution which needs about $H(p)$ bits per letter. You can find this distribution $p$ for English [online](https://en.wikipedia.org/wiki/Letter_frequency); its entropy is $H(p) \approx 4$ bits and there are code names for letters that get close to this entropy (see image below). Since each letter in the original Wikipedia is stored using 8 bits<Footnote>The Wikipedia file uses the [UTF-8 encoding](https://en.wikipedia.org/wiki/UTF-8) which is the standard way to encode text. This encoding stores English letters using 8 bits, obscure letters using 16 bits, and things like 😀, 𓀀, or 𒀱 using 32 bits. This is an example of coding theory in action -- less frequent symbols are stored using more bits. </Footnote>, we have a simple approach to bring the file size down to about 500MB. This is already quite a lot, even without using helpful facts like that 'th' often continues by letter 'e'. 
...
What's the limit on the compression factor, if we don't treat letters independently? 

Claude Shannon, who [came up with information theory](https://en.wikipedia.org/wiki/A_Mathematical_Theory_of_Communication) in late 40's did a pretty cool [experimental work](https://www.princeton.edu/~wbialek/rome/refs/shannon_51.pdf) a few years after that. He took English sentences, cut them at a random place, and asked people to predict the next (missing) letter. This way, he estimated that the entropy of English is at most around 1 bit per letter. That is, we -- humans -- are able to compress text by a factor of around 8, which would correspond to storing 1GB in just over 100MB. Coincidentally, just over 100MB is also the current record in [Marcus Hutter challenge](http://prize.hutter1.net/), so there really seems to be something on "1 bit per letter". 

But not so fast! Large language models are literally trained to predict the next letter (more precisely token) in an English text, so how good are they? The GPT-2 model was around the human-level of prediction with roughly 1 bit per letter, but current models are closer to 0.5 bits per letter. <Footnote>Wait a minute, why is the record for Hutter challenge 100MB and not 50MB then? The problem is that the size of the model itself should also be counted towards the size of the compressed file. So, to use LLMs in your compression algorithm, you would have to train them from scratch inside your compression algorithm and that currently requires way more than 1GB data that we try to compress. </Footnote> Measuring this compression ratio (either as the [cross-entropy loss function](05-machine_learning/neural-nets) or equivalently using so-called [perplexity](https://en.wikipedia.org/wiki/Perplexity)) is in fact one of the most basic ways in which we compare the strength of different models. 

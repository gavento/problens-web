...
KL divergence and entropy are deeply connected to coding theory. There are [tons of great explainers out there](resources), so I only left a quick taste of why this connection matters to the following Expand boxes.  
...
I actually like to think about Kraft's inequality as equality. Why? Well, if your code is such that the left-hand side is really smaller than $1$, then you are stupid. <Footnote>You can notice that Shannon's code above is stupid and leaves some space at the right. In fact, Shannon's code is useful mostly for didactical reasons and in practice you would construct the code by [Huffman's algorithm](https://en.wikipedia.org/wiki/Huffman_coding). </Footnote> You can improve the code like in the picture below and get <Math math="\sum_{i = 1}^k 2^{- \ell_i} = 1" />. 
...
$$\sum_{i = 1}^k p_i \log \frac{1}{q_i} \ge \sum_{i = 1}^k p_i \log \frac{1}{p_i} $$
and plug in <Math math="q_i = 2^{-\ell_i}" />, we get
$$\sum_{i = 1}^k p_i \ell_i \ge H(p). $$
...
The tl;dr is that there is another way to think about entropy. Remember this picture? 

![entropy](02-crossentropy/entropy.png)

We talked about "average surprisal," but we could also ask: how many bits do you need to store coin flip results? On the left, it's 1 bit per flip. On the right, you'd be smart to just store where the rare heads show up—way less than 1 bit per flip on average. 

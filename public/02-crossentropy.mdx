...
More generally, it makes sense to assign code names starting with <Math math = "\textsc{0}" /> to letters with overall frequency $1/2$, and the rest should start with <Math math = "\textsc{1}" />. For example, our code assigned <Math math = "\textsc{0}" /> to A and <Math math = "\textsc{1}" /> is the starting bit for codes of C,G,T. This way of repeatedly splitting the letters in halves leads to code lengths of $\log 1/p$ for letters of frequency $p$. <Footnote>Again, this works if probabilities are powers of two, if not, building good codes gets a bit more technical. </Footnote> 

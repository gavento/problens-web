...
This totally makes sense! Remember, KL divergence tells us how hard it is to tell $p$ apart from $q$ using Bayes' theorem. When you group outcomes together, it gets harder for our Bayesian detective to spot the difference. In the extreme, you could group everything into SOMETHING_HAPPENED—then the inequality becomes $D(p,q) \ge 0$. So nonnegativity is just a special case of this!
</Expand>
...
Here's the intuition: Say you've got 8 equally likely letters. Makes sense to give them all codes of the same length, and that length should be $\log 8 = 3$ to cover everyone. 

More generally, it makes sense to split your alphabet in half by frequency. Letters in the first half get codes starting with <Math math = "\textsf{0}" />, the rest start with <Math math = "\textsf{1}" />. Like how we gave A (50% frequency) the code <Math math = "\textsf{0}" />, while C,G,T share the other half and start with <Math math = "\textsf{1}" />. Keep splitting recursively and you get codes of length $\log 1/p$. <Footnote>Again, this works perfectly when probabilities are powers of two. Otherwise it gets trickier. </Footnote> 
...
Cross-entropy makes sense too: it's the bits you need when data comes from $p$ but you built your code for $q$. And relative entropy? That's just how much worse your mismatched code is compared to the optimal one. 
...
Claude Shannon, who [invented information theory](https://en.wikipedia.org/wiki/A_Mathematical_Theory_of_Communication) in the late 40s, did this cool [experiment](https://www.princeton.edu/~wbialek/rome/refs/shannon_51.pdf) a few years later. He'd show people partial sentences and ask them to guess the next letter. This way, he figured English has at most about 1 bit per letter of entropy. So humans can compress text by a factor of 8, storing 1GB in just over 100MB. Funny thing—that's also the current [Marcus Hutter challenge](http://prize.hutter1.net/) record. Something special about that "1 bit per letter"! 

But hold up! Large language models literally train to predict the next letter (well, token) in text. How good are they? GPT-2 matched humans at roughly 1 bit per letter, but today's models are down to 0.5 bits per letter. <Footnote>So why isn't the Hutter record 50MB? Because you'd have to include the model itself in your compressed file. Training an LLM from scratch inside your compression algorithm would take way more than the 1GB you're trying to compress. </Footnote> We actually use this compression ability (measured as [cross-entropy loss](05-machine_learning/neural-nets) or [perplexity](https://en.wikipedia.org/wiki/Perplexity)) as one of the main ways to compare models. 

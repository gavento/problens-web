...
If we are flipping a fair coin and want to keep results, there is nothing smarter to do then just remember a file like 'HTTHTHTHTTHH' (just exchange H, T to 0, 1) which is 1 bit per flip. 

But if we are flipping a coind where heads have only $0.01$ probability, there are better ways to store the results! For example, using code names $TTTTT \rightarrow 0, H \rightarrow 10, T \rightarrow 11$, we spend around $0.2$ bits per flip! Entropy is the limit on how much space we need to store the results, "how much there is to store". 

With coding theory intuitions, cross-entropy becomes also very natural: it's the bits you need when data comes from $p$ but you built your code for a different distribution $q$. And relative entropy is how much worse your mismatched code is compared to the optimal one. 
